"""
Workstream A2: Re-score steered corpus with Gemini 1.5 Pro (via OpenRouter)
============================================================================
...
Usage:
    modal run modal_judge_gemini.py
    modal run modal_judge_gemini.py --batch-size 100
"""
...
JUDGE_MODEL = "google/gemini-2.5-flash"       # Upgraded for better ICC
...
OUTPUT_PATH_VOLUME = "/results/judge_scores_gemini.json"
...
app = modal.App("judge-rescoring-gemini", image=image)
...
def score_batch(batch: dict) -> list:
    """
    Score a batch of responses using Gemini 1.5 Pro via OpenRouter.
...
def main(batch_size: int = BATCH_SIZE, limit: int = 0, rescore: bool = False):
    """
    Re-score all 1,200 steered responses with Gemini.

    Usage:
        modal run modal_judge_gemini.py
        modal run modal_judge_gemini.py --batch-size 50
        modal run modal_judge_gemini.py --limit 5   # test with 5 responses
        modal run modal_judge_gemini.py --rescore    # re-score only blocked items
    """
    # ------------------------------------------------------------------
    ...
    if rescore:
        local_output_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "results", "judge_scores_gemini.json",
        )
        ...
        # Heuristic fallback for old data without blocked field
    ...
    print(f"\n{'=' * 70}")
    print(f"WORKSTREAM A2: JUDGE RE-SCORING WITH {JUDGE_MODEL}")
    print(f"{'=' * 70}")

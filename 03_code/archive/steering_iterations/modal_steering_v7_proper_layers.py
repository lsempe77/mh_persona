"""
V7: Proper Layer Selection for Persona Vector Extraction
=========================================================

FIXES:
1. v5 bug: Circular measurement (projecting steered output onto steering vector)
2. v6 bug: All layers = 31 (raw separation favors later layers with higher magnitude)

PROPER LAYER SELECTION: Use cosine-normalized separation (Cohen's d style)
This normalizes by the pooled standard deviation so later layers don't win by magnitude alone.

From Chen et al. (2025): "Layer selection should maximize discriminability, 
...
# Modal setup
app = modal.App("steering-v7-proper-layers")
...
# Extended training prompts - 10 per trait
TRAINING_PROMPTS = {
    "empathy": [
...
def run_v7_proper_layers():
    """
    V7: Proper layer selection using Cohen's d (normalized separation).
    """
    import torch
    import numpy as np
    ...
    print("="*70)
    print("V7: PROPER LAYER SELECTION")
    print("="*70)
    print("\nFIXES:")
    print("1. Independent scoring (not circular)")
    print("2. Cohen's d for layer selection (normalized, not raw magnitude)")
    print()
    ...
    for trait_name, pairs in TRAINING_PROMPTS.items():
        ...
        # Try middle and upper-middle layers (10-28) - skip very early and last layers
        # Research shows middle layers (10-20) often best for steering
        for layer_idx in range(10, 29):  # Layers 10-28 (skip 0-9 and 29-31)
            layer_directions = []
            pos_projections = []
            neg_projections = []
    ...
    for trait, info in vectors.items():
        ...
        print(f"  {trait}: layer {layer} (d={info['cohens_d']:.2f})")
    ...
    # Save results
    output = {
        "version": "v7_proper_layers",
        "fixes": [
            "Independent scoring (not circular)",
            "Cohen's d for layer selection (normalized)",
            "Restricted layer range (10-28) based on literature"
        ],
        ...
        "sample_responses": results[:30],
    }
    
    with open("/results/v7_proper_layers_results.json", "w") as f:
        json.dump(output, f, indent=2)
    
    vol.commit()
    
    print("\n✓ Results saved to /results/v7_proper_layers_results.json")
...
def main():
    result = run_v7_proper_layers.remote()
    
    print("\n" + "="*70)
    print("V7 PROPER LAYERS COMPLETE")
    print("="*70)

"""
V21: Polarity Fixes + Layer 15 + Improved Contrasts

KEY INSIGHTS FROM V20:
- Fixed layer 14 achieved 3/10 working (vs V19's 0/10)
- Working: inappropriate_self_disclosure (0.563), crisis_recognition (0.478), empathetic_responsiveness (0.459)
- INVERTED: boundary_maintenance (-0.411), sycophancy (-0.324), harmful_advice (-0.258)
- ZERO EFFECT: grounded_calmness (0.036), emotional_over_involvement (0.116), abandonment (0.150)

V21 CHANGES:
1. Layer 15 (V18 showed empathy worked great at layer 15)
2. FLIP POLARITY for 3 inverted traits (swap high/low prompts)
3. REWRITE contrast examples for 3 zero-effect traits
4. Keep coefficients [-3, -1.5, 0, 1.5, 3]

EXPECTED: 6-7/10 working traits
"""
...
app = modal.App("steering-v21-polarity-fixes")
vol = modal.Volume.from_name("steering-results", create_if_missing=True)

# ==========================================================================
# CONFIGURATION - LAYER 15 (V18 showed empathy r=0.732 at layer 15)
# ==========================================================================

STEERING_LAYER = 15  # Layer 15 worked well for empathy in V18
...
TRAITS = {
    ...
    # REWRITTEN - V20 showed r=0.036 (zero effect) - need stronger contrast
    "grounded_calmness": {
    ...
    # POLARITY FLIPPED BACK - V20 still showed r=-0.324 (inverted!)
    ...
    # REWRITTEN - V20 showed r=0.150 (weak) - need stronger, more concrete contrast
...
def generate_for_trait(trait_name: str):
    """Generate steered responses for one trait at layer 15."""
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig
    from tqdm import tqdm
    
    trait_config = TRAITS[trait_name]
    layer_idx = STEERING_LAYER  # Layer 15
...
def main():
    """V21: Polarity fixes + Layer 15 + Improved contrasts."""
    print("="*70)
    print("V21: POLARITY FIXES + LAYER 15 + IMPROVED CONTRASTS")
    print("="*70)
    print(f"Layer: {STEERING_LAYER}")
    print(f"Coefficients: {STEERING_COEFFS}")
    print(f"Traits: {len(TRAITS)}")
    print(f"Prompts: {len(VALIDATION_PROMPTS)}")
    print("Polarity flipped: boundary_maintenance, harmful_advice_propensity, sycophancy")
    print("Contrasts rewritten: grounded_calmness, emotional_over_involvement, abandonment")
    print()
    ...
    # Save results
    results = {
        "version": "v21_polarity_fixes",
        "methodology": {
            "extraction": "last-token",
            "layer": STEERING_LAYER,
            "layer_selection": "Layer 15 (V18 empathy success)",
    ...
    # Save to volume - ensure directory exists
    import os as os_module
    os_module.makedirs("/results", exist_ok=True)
    with open("/results/v21_polarity_fixes_results.json", "w") as f:
        json.dump(results, f, indent=2)
    vol.commit()
    
    print(f"\n✓ Results saved to v21_polarity_fixes_results.json")

...
# V15: ACTIVATION STEERING (not logit steering)
# Based on V9 which achieved 2/10 working traits using activation steering
# Coefficients match V9's successful settings
STEERING_COEFFS = [-3.0, -1.5, 0.0, 1.5, 3.0]  # V9 coefficients
SEED = 42
USE_GREEDY_DECODING = False  # V9 used sampling
MAX_NEW_TOKENS = 150
TEMPERATURE = 0.7
TOP_P = 0.9  # V9 parameter
...
def generate_responses_for_trait(trait_name: str, trait_config: dict):
    ...
    # ========================================================================
    # V16 FIX: USE PER-TRAIT LAYER INDICES (matching V9 which had 2/10 working)
    # ========================================================================
    # V9 analysis showed different traits are encoded at different layers:
    # - non_judgmental_acceptance (r=0.375, p=0.007) used layer 14
    # - abandonment_of_therapeutic_frame (r=0.322, p=0.022) used layer 16
    # Using late layers (30) for all traits didn't work in V15
    LAYER_MAP = {
        "empathetic_responsiveness": 26,       # From V9
        "non_judgmental_acceptance": 14,       # V9 working trait!
        "boundary_maintenance": 17,            # From V9
        "crisis_recognition": 17,              # From V9
        "harmful_advice_propensity": 25,       # From V9
        "sycophancy_harmful_validation": 19,   # From V9
    }
    
    DEFAULT_LAYER = 17  # Middle layer fallback
    EXTRACTION_LAYER = LAYER_MAP.get(trait_name, DEFAULT_LAYER)
    print(f"Extracting steering vectors from layer {EXTRACTION_LAYER} (per-trait: {trait_name})")
    ...
    # ========================================================================
    # FIX 3: Validate vector dimensionality matches model hidden size
    # ========================================================================
    model_hidden_size = model.config.hidden_size
    if steering_vector.shape[0] != model_hidden_size:
        raise ValueError(f"Steering vector dim {steering_vector.shape[0]} != model hidden size {model_hidden_size}")
    
    print(f"    ✓ Vector extracted (dim={steering_vector.shape[0]}, matches model hidden size)")
    
    # ========================================================================
    # ACTIVATION STEERING SETUP
    # ========================================================================
    # Move steering vector to device for activation steering
    steering_vec_device = steering_vector.to(model.device).to(model.dtype if hasattr(model, 'dtype') else torch.float16)
    
    # Compute steering magnitude statistics for logging
    vec_norm = steering_vec_device.norm().item()
    print(f"    Steering vector norm: {vec_norm:.4f}")
    print(f"    Steering coefficients: {STEERING_COEFFS}")
    print(f"    Effective steering range: [{vec_norm * min(STEERING_COEFFS):.2f}, {vec_norm * max(STEERING_COEFFS):.2f}]")
    
    # ========================================================================
    # GENERATION FUNCTION
    # ========================================================================
    
    def generate_with_activation_steering(prompt: str, coeff: float):
        """Generate with activation steering (adds steering vector to hidden states).
        This method modifies hidden states at the extraction layer during the forward pass,
        affecting all subsequent layers and the final output distribution.
        """
        ...
        def steering_hook(module, input, output):
            ...
            steering = coeff * steering_vec_device.to(hidden.dtype)
            hidden[:, :, :] = hidden + steering
            if isinstance(output, tuple):
                return (hidden,) + output[1:]
            return hidden
        
        # Register hook at the extraction layer
        handle = model.model.layers[EXTRACTION_LAYER].register_forward_hook(steering_hook)
        ...
        try:
            ...
            handle.remove()  # Always remove the hook
    ...
    for coeff in tqdm(coeffs_to_test, desc=f"  {trait_name}"):
        for prompt in VALIDATION_PROMPTS:
            response = generate_with_activation_steering(prompt, coeff)
    ...
    # Ablation: Random direction baseline (using activation steering with random vector)
    if not freeze_steering:
        print(f"\n  Generating ABLATION (random direction)...")
        random_vector = torch.randn_like(steering_vector)
        random_vector = random_vector / (random_vector.norm() + 1e-8)  # Normalize
        random_vec_device = random_vector.to(model.device)
        
        def generate_with_random_steering(prompt: str, coeff: float):
            """Generate with random activation steering for ablation."""
            formatted = f"[INST] {prompt} [/INST]"
            inputs = tokenizer(formatted, return_tensors="pt", truncation=True, max_length=512)
            inputs = {k: v.to(model.device) for k, v in inputs.items()}
            ...
            def random_hook(module, input, output):
                hidden = output[0] if isinstance(output, tuple) else output
                steering = coeff * random_vec_device.to(hidden.dtype)
                hidden[:, :, :] = hidden + steering
                if isinstance(output, tuple):
                    return (hidden,) + output[1:]
                return hidden
            
            handle = model.model.layers[EXTRACTION_LAYER].register_forward_hook(random_hook)
        ...
        for coeff in [1.0, -1.0]:
            for prompt in VALIDATION_PROMPTS[:5]:
                response = generate_with_random_steering(prompt, coeff)
                safety_result = safety_check(response, prompt)
...
def analyze_results(all_data: list):
    ...
    for trait_data in all_data:
        ...
        metadata[trait_name] = {
            ...
            "steering_stats": trait_data.get("steering_stats"),
        }
    ...
    for trait_name in main_df["trait"].unique():
        ...
        # V15: Use extreme coefficients from V9 coefficient list
        # With STEERING_COEFFS = [-3.0, -1.5, 0.0, 1.5, 3.0], extremes are -3.0 and 3.0
    ...
    # Save results
    output = {
        "version": "v16_per_trait_layers",
        "method_description": "V16: Per-trait layer indices (matching V9 which had 2/10 working traits). Different traits are encoded at different layers - V9's working traits used early-mid layers (14-16), not late layers.",
        "fixes_applied": [
            "1. Full determinism (CUDA seeds, cuDNN flags)",
            "2. Sentinel tokens (<CHOICE_A>, <CHOICE_B>) verified single-token",
            "3. V16: Per-trait layer indices (empathy:26, non-judgmental:14, boundary:17, crisis:17, harmful:25, sycophancy:19)",
            "4. Defensive model attribute checks",
            "5. Activation steering (hidden state modification) instead of logit steering",
            "6. Sampling mode (matching V9 which had 2/10 working traits)",
            "7. Judge retry with exponential backoff",
            "8. Analysis guards for empty data",
            "9. Safety filter implementation",
            "10. Audit logging",
            "11. Coefficients match V9: [-3.0, -1.5, 0, 1.5, 3.0]",
            "12. V16: Layer selection based on V9 analysis - working traits used early-mid layers",
        ],
        "config": {
            "seed": SEED,
    ...
    # Save main results
    with open("/results/v16_per_trait_layers_results.json", "w") as f:
    ...
    vol.commit()
    print("\n✓ Results saved to /results/v16_per_trait_layers_results.json")
    if all_audit_logs:
    ...
    for trait_name in TRAITS.keys():
        if trait_name in generation_map and trait_name in judged_map:
            ...
            full_data.append({
                ...
                "steering_stats": gen_out.get("steering_stats"),
            })

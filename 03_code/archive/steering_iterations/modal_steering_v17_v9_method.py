"""
V17: Using V9's Proven Methodology with 6 Core Traits
======================================================

V9 had 2/10 working traits (non_judgmental_acceptance r=0.375, abandonment r=0.322).
V15/V16 with sentinel tokens and contrast pairs had 0/6 working.

Key V9 methodology differences that worked:
1. Last token position for activation extraction (not sentinel tokens)
2. Simple high/low prompt format (not complex contrast pairs)
3. Direct high_act - low_act vector computation

V17 returns to V9's proven methodology, focused on 6 core traits.

Parallelization: 6 traits × 6 parallel GPU workers
...
# Modal setup
app = modal.App("steering-v17-v9-method")
...
def generate_responses_for_trait(trait_name: str, trait_config: dict):
    ...
    # Extract steering vector - 6 core traits only
    # Using layer indices from V9 which had working traits
    LAYER_MAP = {
        "empathetic_responsiveness": 26,
        "non_judgmental_acceptance": 14,  # V9 working trait (r=0.375)!
...
def analyze_results(all_results: list):
    ...
    with open("/results/v17_v9_method_results.json", "w") as f:
        json.dump(output, f, indent=2)
    
    vol.commit()
    
    print("\n✓ Results saved to /results/v17_v9_method_results.json")

"""
Workstream A0: Score steered corpus with GPT-4o-mini (via OpenRouter)
======================================================================
Generate GPT-4o-mini baseline scores for the 1,200 responses from
steered_corpus_combined.json, matching the rubric used by the Claude
and Gemini judges for 3-way ICC comparison.

Usage:
    modal run modal_judge_gpt4o_mini.py
    modal run modal_judge_gpt4o_mini.py --limit 5
"""
...
JUDGE_MODEL = "openai/gpt-4o"                       # Upgraded for better ICC
...
OUTPUT_PATH_VOLUME = "/results/judge_scores_gpt4o_mini.json"
...
app = modal.App("judge-rescoring-gpt4o-mini", image=image)
...
def main(batch_size: int = BATCH_SIZE, limit: int = 0, rescore: bool = False):
    ...
    if rescore:
        local_output_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "results", "judge_scores_gpt4o_mini.json",
        )
